...
I. Input:
	...
	2. Input from another table
		insert into (table_name)(columns) select (columns_from_another_table )from (another_table_name) where /condition/ 
II. Output:
	Select (.) from (table)
	1. (.) = * : All table
...
IV. Alter table
	1. Add column
		alter table (table_name) add column (new_column_name) (datatype) // const / default + (values)
	2. Change datatype
	...
	5. Set NULL
		alter table (table_name) alter column set (type)
V. Constraint
	1. Unique
		alter table (table_name) add constraint unique_(column) unique((column))	
	2. Primary Key
		alter table (table_name) add constraint (column)_pk primary key((column))
	3. Foreign Key
		...
		- alter table child_table_name
		  add constraint (child_table_name)_fkey foreign key(adult_table_name.id) references adult_table_name(id)
	4. Check
		- create table (table_name)
			(
				column_name datatype check (condition)
			);
		- alter table (table_name) add constraint (column1)_(column2)_.._check
			check(condition)
	5. Drop Constraint
...
VII. Delete
	1. Clear table
		...
		- truncate table (table1_name), (table2_name),.. // can be add cascade to remove foreign key
		-> truncate is faster
...
IX. Select Statement
	1. Name alias
		- For column: select (column_name) as (name_alias) from (table_name)
		- For table: select (list_column) from (table_name) as (name_alias)
...
X. Filter the data
	...
	2. Like and Not Like and Similar to and ilike
		- where (column_name) like/not like '(...)'
			(...) : % (string) / _ (char)
			exp: Khuong -> 'K%' / '%uon%' / '%ng'...
		- where (column_name) similar to '%(values1|values2|...)%'
	...
	4. Limit
		...
		- will return (n) rows
	5. Offset
		select * from (table_name) order by (table_name) limit (n) offset (m)
		- pass m rows
	6. Is null & is not null
		values is null / is not null
XI. Group
	1. Inner join (select values from 2 table)
		select A.(Column_name),..., (Column_name_of_B),... from A inner join B on (addition)
	2. Left join
		select A.pka, A.c1, B.pkb, B.c2 from A left join B on (addition)
